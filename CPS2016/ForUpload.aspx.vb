...
Public Class ForUpload
    ...
    Private Sub AddPODetails(ByVal DAL As DAL, ByVal dt As DataTable, ByVal intPOID As Integer, ByVal strBatch As String, _
        ...
        For Each rw As DataRow In dt.Rows
            ...
            If DAL.AddRelPOData(intPOID, rw("CRN"), rw("Barcode"), rw("FirstName"), rw("MiddleName"), rw("LastName"), rw("Suffix"), rw("Sex"), rw("DateOfBirth"), rw("Address"), ActivityID, strBackOCR, 0, 0, strSubDir, rw("AddressDelimited")) Then
                ...
                'If Not DAL.AddRelPOImage_PhotoAndSignature(rw("Barcode"), _
                '                            File.ReadAllBytes(String.Format("{0}\{1}_Photo.jpg", rw("Path"), rw("Barcode"))), _
                '                            GetFileByte(String.Format("{0}\{1}_Signature.tiff", rw("Path"), rw("Barcode")), 2), _
                '                            intPOID) Then
                '    IsSuccess = False
    ...
    Private Sub AddPODetails2(ByVal DAL As DAL, ByVal dt As DataTable, ByVal intPOID As Integer, ByVal strBatch As String, _
                             ByRef intBoxCntr As Integer, ByRef intBoxSeriesCntr As Integer, ByRef intDtlCntr As Integer, _
                             ByRef intPageCntr As Short, ByRef intPageSeriesCntr As Short, ByRef IsSuccess As Boolean, _
                             ByRef intError As Integer, ByRef sb As StringBuilder, _
                             Optional ActivityID As DataKeysEnum.ActivityID = DataKeysEnum.ActivityID.IndigoDownload)
        For Each rw As DataRow In dt.Rows
            ...
            If DAL.AddRelPOData(intPOID, rw("CRN"), rw("Barcode"), rw("FirstName"), rw("MiddleName"), rw("LastName"), rw("Suffix"), rw("Sex"), rw("DateOfBirth"), rw("Address"), ActivityID, strBackOCR, 0, 0, strSubDir, rw("AddressDelimited")) Then
                ...
                If Not DAL.AddRelPOImage(rw("Barcode"), _
                                            File.ReadAllBytes(String.Format("{0}\{1}.xml", rw("Path"), rw("Barcode"))), _
                                            File.ReadAllBytes(String.Format("{0}\{1}_perso.xml", rw("Path"), rw("Barcode"))), _
                                            File.ReadAllBytes(String.Format("{0}\{1}_Photo.jpg", rw("Path"), rw("Barcode"))), _
                                            File.ReadAllBytes(String.Format("{0}\{1}_Signature.tiff", rw("Path"), rw("Barcode"))), _
                                            File.ReadAllBytes(String.Format("{0}\{1}_Rprimary.ansi-fmr", rw("Path"), rw("Barcode"))), _
                                            File.ReadAllBytes(String.Format("{0}\{1}_Rbackup.ansi-fmr", rw("Path"), rw("Barcode"))), _
                                            File.ReadAllBytes(String.Format("{0}\{1}_Lprimary.ansi-fmr", rw("Path"), rw("Barcode"))), _
                                            File.ReadAllBytes(String.Format("{0}\{1}_Lbackup.ansi-fmr", rw("Path"), rw("Barcode")))) Then
                    IsSuccess = False
                    intError += 1
                    sb.AppendLine(String.Format("AddRelPOImage(): Barcode {0}, CRN {1}. Returned error {2}<br>", rw("Barcode").ToString.Trim, rw("CRN").ToString.Trim, DAL.ErrorMessage))
                End If
    ...
    Private Function GetFile(ByVal strFile As String, ByVal intType As Short) As String
        If File.Exists(strFile) Then
            Return strFile
        Else
            Select Case intType
                Case 1 'xml
                    Return SharedFunction.templateXML_File
                Case 2 'signature
                    Return SharedFunction.templateSignature_File
                Case 3 'ansi
                    Return SharedFunction.templateANSI_File
            End Select
        End If
    End Function
    ...
    Private Function GetTotalQtyOfZipFile(ByVal zipFile As String) As Integer
    ...
    Private Sub UploadData(ByVal strPurchaseOrder As String, ByVal strBatch As String, ByVal strFile As String)
        Dim ForUploadingFileType As String = SharedFunction.GetForUploadingFileType

        If ForUploadingFileType = "" Then
            ...
            GridDataBind()
        Else
            Dim encryptedFile As String = String.Format("{0}\{1}", SharedFunction.ForUploadingRepository, strFile)
            Dim outputZipFile As String = ""
            Dim response As String = DecryptData(encryptedFile, outputZipFile)
            If response = "" Then
                If File.Exists(outputZipFile) Then
                    If File.Exists(outputZipFile) Then
                        If UnzipFile(outputZipFile, String.Format("{0}\{1}", SharedFunction.ForUploadingRepository, strPurchaseOrder)) Then
                            ASPxMemo1.Visible = False

                            Dim sb As New StringBuilder

                            If SharedFunction.UploadData_v2(strPurchaseOrder, strBatch, encryptedFile, SharedFunction.GetCurrentPage(Request.Url.AbsolutePath), SharedFunction.UserID(Page.User.Identity.Name), sb) Then
                                'housekeeping. delete encryptedFile and outputZipFile file
                                'File.Delete(encryptedFile)
                                'IO.File.Delete(outputZipFile)
                            End If
                    ...
                    Else
                        ASPxMemo1.Visible = True
                        ASPxMemo1.Text = "Unable to find " & strPurchaseOrder & " zip file"
                    End If
                Else
                    ASPxMemo1.Visible = True
                    ASPxMemo1.Text = "Unable to find " & encryptedFile & " file"
                End If
            Else 'decrypt failed
                ASPxMemo1.Visible = True
    ...
    Private Sub UploadExtractedData() ', ByVal ID2 As Integer)       
        ...
        For Each strSubDir As String In Directory.GetDirectories(SharedFunction.ForUploadingRepository & "\" & strPurchaseOrder)
            For Each strSubDir2 As String In Directory.GetDirectories(strSubDir)
                ...
                Try
                    MemberXML = New MemberXML(String.Format("{0}\{1}.xml", strSubDir2, _barcode))
    ...
    Private Sub BindGrid()
        ...
        If ForUploadingFileType = "" Then
            For Each strSubDir In Directory.GetDirectories(SharedFunction.ForUploadingRepository)
                Try
                    PO = strSubDir.Substring(strSubDir.LastIndexOf("\") + 1)

                    If Not IsPOExist(PO, dtUploaded) Then
                        Dim rw As DataRow = dtForUpload.NewRow
                        rw("PurchaseOrder") = PO 'PO
                        rw("Batch") = PO.Split("-")(3)
                        rw("DateTimePosted") = New DirectoryInfo(strSubDir).CreationTime
                        rw("Quantity") = 0
                    ...
                    End If
                Catch ex As Exception
                    DAL.AddErrorLog("PopulatingFolder: Error encountered in " & PO, SharedFunction.GetCurrentPage(Request.Url.AbsolutePath), SharedFunction.UserID(Page.User.Identity.Name))
                End Try
            Next
        Else
            For Each strFile In Directory.GetFiles(SharedFunction.ForUploadingRepository)
                Try
                    ...
                    PO = Path.GetFileNameWithoutExtension(strFile).Substring(0, 33)

                    If orig_file.Contains(ForUploadingFileType) Then
                        If Not IsPOExist(PO, dtUploaded) Then
    ...
    Protected Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        'Dim response As String = DecryptData(String.Format("{0}\{1}", SharedFunction.ForUploadingRepository, "01-20130122-P-023-4249-C-001-4053_zip.sss"), outputFile)
        UploadExtractedData()
    End Sub

    'Private Sub UploadData_orig(ByVal strPurchaseOrder As String, ByVal strBatch As String, ByVal strFile As String) ', ByVal ID2 As Integer)
    '    Dim outputFile As String = ""
    ...
    'Private Sub UploadData2(ByVal strPurchaseOrder As String, ByVal strBatch As String, ByVal strFile As String) ', ByVal ID2 As Integer)
    '    Dim outputFile As String = ""
    '    Dim response As String = DecryptData(String.Format("{0}\{1}", SharedFunction.ForUploadingRepository, strFile), outputFile)
    '    If response = "" Then
    '        If UnzipFile(outputFile, String.Format("{0}\{1}", SharedFunction.ForUploadingRepository, strPurchaseOrder)) Then
